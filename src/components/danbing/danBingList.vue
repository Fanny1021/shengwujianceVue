    ...
    export default {
        ...
        methods: {
            ...
            query:function () {
                ...
                this.$api.user.$getJson('sys/soldier/list',jsondata).then((resp)=>{
                    this.loading = false;
            ...
            deleteRowClick(index,row){alert(row.biologicId)
                this.$api.user.$getJson('sys/clear/{biologyId}',JSON.stringify(row.biologicId)).then((resp)=>{
                    this.loading = false;
                    if (resp && resp.status == 200) {
                        //返回的json数据
                        var data = resp.data;
